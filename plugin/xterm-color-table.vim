...
function! s:create_table()
	...
	call add(rows, s:color_row(0,  7))
	call add(rows, s:color_row(8, 15))
	call add(rows, '')

	for lnum in range(16, 250, 6)
		call add(rows, s:color_row(lnum, lnum + 5))
		if lnum == 226
			call add(rows, '')
		endif
	endfor
	...
	setlocal nomodified nomodifiable noswapfile readonly nocursorline nocursorcolumn nospell
	setlocal iskeyword+=#
endfunction

function! s:color_row(start, end)
	return join(map(range(a:start, a:end), 's:color_cell(v:val)'))
endfunction

function! s:color_cell(n)
	...
	execute 'highlight fg_' . a:n . ' ctermfg=' . a:n . ' guifg=' . s:xterm_colors[a:n]
	execute 'highlight bg_' . a:n . ' ctermbg=' . a:n . ' guibg=' . s:xterm_colors[a:n]
	execute 'highlight bg_' . a:n . ' ctermfg=' . a:n . ' guifg=' . s:xterm_colors[a:n]
